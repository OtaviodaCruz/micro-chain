...
void ContentStore::onIngressInterest(const std::shared_ptr<Face> &ingress_face, const ndn::Interest &interest) {
    ...
    if (entry) {
        ...
        logger::log(logger::INFO, "respond with cache");
...
void ContentStore::commandEditConfig(const rapidjson::Document &document) {
    ...
    // {"action": "edit_config", "id": 10
    // , "manager_address": "", "manager_port": "", "report_each": "", "strategy":}
...
void ContentStore::commandReport(const boost::system::error_code &err) {
    std::cout <<  "[Report] " << "hit_count: " << _hit_counter << " miss_count: " << _miss_counter;
    
    if (!err){ //&& _monitor_endpoint.address() != boost::asio::ip::address_v4::any() && _monitor_endpoint.port() != 0) {
        std::stringstream ss;
        ss << R"({"name":")" << _name << R"(", "type":"report", "action":"cache_status", "hit_count":)" << _hit_counter << R"(, "miss_count":)" << _miss_counter << "}";
        _command_socket.send_to(boost::asio::buffer(ss.str()), _monitor_endpoint);
        //_miss_counter=0
        //_hit_counter=0
    }
    if(_report_enable) {
...
void ContentStore::DefineEndPointMonitor(){

        std::cout << "Define monitor end point" << "\n";

        boost::asio::ip::udp::endpoint new_endpoint(boost::asio::ip::address::from_string("127.0.0.1"), 9999);
        
        _monitor_endpoint = new_endpoint;
