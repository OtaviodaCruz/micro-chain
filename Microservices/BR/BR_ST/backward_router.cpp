...
void BackwardRouter::onIngressData(const std::shared_ptr<Face> &ingress_face, const ndn::Data &data) {
    // drop
    commandReportErrorFace(ingress_face);
}

void BackwardRouter::onEgressInterest(const std::shared_ptr<Face> &egress_face, const ndn::Interest &interest) {
    // drop
    commandReportErrorFace(egress_face);
}

void ContentStore::commandReportErrorFace(const std::shared_ptr<Face> &face) {
    logger::log(logger::INFO, "[Face Error] Package droped");
    try {
        if (_manager_endpoint.address() != boost::asio::ip::address_v4::any() && _manager_endpoint.port() != 0){ 
            size_t faceID = face->getFaceId();

            std::stringstream ss;        
            ss << R"({"name":")" << _name << R"(", "type":"report", "action":"face_error", "face_id":)" << faceID << "}";

            _command_socket.send_to(boost::asio::buffer(ss.str()), _manager_endpoint);
        }
    }
    catch (...) {
        logger::log(logger::INFO, "[catch] Error on running commandReportErrorFace");
    }
}
