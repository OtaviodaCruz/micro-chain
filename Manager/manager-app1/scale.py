...
def autoScale(socketObj, graphObj):
    ...
    for name, attrs in list(graphObj.nodes(data=True)):
        if attrs["scalable"] and name not in graph.locked_nodes:
            graph.locked_nodes.add(name)
            decision = scaleDecision(graphObj, name, attrs)
            if decision == 1:
                log.printWithColor("[ autoScale ] scale up " + name, type="INFO")
                yield scale_up_functions.get(attrs["type"], scaleUp)(socketObj, graphObj, name, attrs)
            elif decision == 2:
                log.printWithColor("[ autoScale ] scale down " + name, type="INFO")
                yield scale_down_functions.get(attrs["type"], scaleDown)(socketObj, graphObj, name, attrs)
            elif decision == 0:
...
def choose_node(typeNode, microName, typeOperation):
    if typeNode == "sr":
        return "otaviopc1-270e5j-2570ej"
    if typeNode == "br":
        return "otaviopc2-270e5j-2570ej"
    else:
        return None

@defer.inlineCallbacks
def scaleUpBR(socketObj, graphObj, name, attrs):
    log.printWithColor("[ scaleUpBR ] start --------------", type="INFO")

    scale = attrs.get("scale", 1)
    in_node_list = list(graphObj.predecessors(name))
    ...
    if scale == 1:
        for i in range(len(in_node_list)):
            ...
            node_for_deploy = choose_node("sr", lb_name, "scale_up")
            if operations.createPodAndNode(lb_name, "sr", graphObj, socketObj, is_scalable=False, nodeName=node_for_deploy):
                ...
                resp = yield operations.createFaceAndEdge(graphObj, socketObj, [lb_name], [name])
                if resp:
                    lb_nodes_list.append(lb_name)
                    resp = yield operations.createFaceAndEdge(graphObj, socketObj, [in_node_list[i]], [lb_name])
                    if resp:
                        resp = yield operations.delFaceAndEdge(graphObj, socketObj, [in_node_list[i]], [name])
                        if resp:
                            pass
                        else:
                            log.printWithColor("[ scaleUpBR ] When delete link " + in_node_list[i] + " to " + name + " Step 2 aborded", type="INFO")

                            yield operations.delFaceAndEdge(graphObj, socketObj, in_node_list[0:i], [lb_name])
                            yield operations.createFaceAndEdge(graphObj, socketObj, in_node_list[0:i-1], [lb_name])
    ...
    node_for_deploy = choose_node("br", clone_name, "scale_up")
    if operations.createPodAndNode(clone_name, attrs["type"].lower(), graphObj, socketObj, is_scalable=False, nodeName=node_for_deploy):
        ...
        for out_name in out_node_list:
            ...
            if resp and resp > 0:
                ...
                graphObj.add_edge(clone_name, out_name, face_id=resp)
                ...
                yield operations.delFaceAndEdge(graphObj, socketObj, in_node_list, [lb_name])
                yield operations.createFaceAndEdge(graphObj, socketObj, in_node_list, [name])
        ...
        for idx, lb_name in enumerate(lb_nodes_list): # for lb_name in lb_nodes_list  ALTERADO
            ...
            if resp and resp > 0:
                ...
                graphObj.add_edge(lb_name, clone_name, face_id=resp)
            else:
                ...
                yield operations.delFaceAndEdge(graphObj, socketObj, lb_name[0:idx], [clone_name]) # preciso confererir
